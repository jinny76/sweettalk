 ...
 */
        ...
        //执行测试列表
        try {
            new TestOAuthV2AuthorizeCodeGrantAndAPI().testDoc();
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    private void testDoc() {
        ...
        try {
            ...
            String json = statusesAPI.homeTimeline(oAuth, format, pageflag, pagetime, reqnum, type, contenttype);
            /*statusesAPI.mentionsTimeline(oAuth, format, pageflag, pagetime, reqnum, lastid, type, contenttype);
            statusesAPI
                .usersTimeline(oAuth, format, pageflag, pagetime, reqnum, lastid, names, fopenids, type, contenttype);
            statusesAPI
                .userTimelineIds(oAuth, format, pageflag, pagetime, reqnum, lastid, name, fopenid, type, contenttype);
            statusesAPI
...
*/
        ...
        response = tAPI.addPic(
            oAuth, format, "发表一条带网络图片的微博" + content, clientip, jing, wei,
            "http://t3.qpic.cn/mblogpic/d26d1168b2c6c25db192/460", syncflag);
        //取出返回的 id
        responseJsonObject = JSONObject.fromObject(response);
        dataJsonObject = (JSONObject) responseJsonObject.get("data");
        ...
        tAPI.addVideo(
            oAuth, format, "发表视频微博" + content, clientip, jing, wei,
            "http://www.tudou.com/programs/view/yx41TA6rQfE/?resourceId=0_03_05_07", syncflag);
        try {
            Thread.sleep(5000);
        } catch (InterruptedException ie) {
        }
...
*/
        ...
        searchAPI.t(
            oAuth, format, keyword, pagesize, page, contenttype, sorttype, msgtype, searchtype, starttime, endtime,
            province, city, longitue, latitude, radius);
...
//---------------------------------------------- 以下为测试辅助方法------------------------------------------------	
    ...
    private static void init(OAuthV2 oAuth) {
        oAuth.setClientId("801325266");
        oAuth.setClientSecret("f5d58d16d79a601d3dec549d291ffaaa");
        oAuth.setRedirectUri("http://www.tencent.com/zh-cn/index.shtml");
        //code=3e0b767136a1c6a9d362c64a059691ab&openid=09DBFB8C953A11CE38B44C13D23DAB23&openkey=7158980C5AD9D301D67708A1A54D10F6
        oAuth.setAccessToken("6ddc44509d33e3bc9464d63b7c6afb2a");
        oAuth.setOpenid("7661A5A81FF61B6C44C8F151088C29A9");
        oAuth.setOpenkey("CFE004A510ADC7FD60DBE6F44DAB7F6E");
